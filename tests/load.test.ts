...
const UPLOAD_COMPLETE_ENDPOINT =
  "http://localhost:3000/grayscale/uploads/complete";
...
function pollJobStatus(jobId) {
  ...
  while (Date.now() - startedAt < STATUS_POLL_TIMEOUT_SECONDS * 1000) {
    ...
    if (
      statusPayload.data.status === "completed" ||
      statusPayload.data.status === "failed"
    ) {
  ...
  fail(
    `job ${jobId} did not reach a terminal state within ${STATUS_POLL_TIMEOUT_SECONDS} seconds`,
  );
}
